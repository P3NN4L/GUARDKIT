...
| **guard-kit 雷达 v7（端内 852KB）** | **100%**（12/12，全高危档，校准分≥0.9） | **0%（0/8）** | **100%** | 0.04ms · 离线 · 免费 |
| MiniMax abab6.5s few-shot（云端） | 100% | 12.5% | 95.0% | 秒级 · 联网 · 按次计费 |
| MiniMax-M3 few-shot（旗舰，云端） | 100% | 0% | 100% | 秒级推理 · 联网 · 按次计费 |

> 结论供参考：旗舰大模型在无歧义真实案例上可达完美，但延迟/成本/隐私不可用于端内海量筛查；端内小模型 + 领域语料 + 校准在 0.04ms 内达到 **100%/100%**（v7 与 M3 few-shot 并列满分，且离线免费）。欢迎用你的系统复测并引用。
>
> 历史版本（同卷可审计）：v6.0 = 100% / 12.5% / 95.0%；v6.1 = 100% / 12.5% / 95.0%；v6.2 = 100% / 0% / 100%；v7 = 100% / 0% / 100%。
