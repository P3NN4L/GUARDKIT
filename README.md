...
**端内反诈打分引擎（雷达 v6.2 · 裁判 v5）**——两个自训小模型（雷达 + 裁判），**可实现无缝嵌入各类 App**：原生 iOS/Android、RN/Expo、Flutter、小程序、网页与云端。纯函数 + 内置权重，零依赖、完全离线、单条推理 <0.1ms，同一份权重六个平台分数逐位一致。本仓库只放接入文档；**引擎源码、模型权重与训练管线在主仓库**（见文末链接）。
...
- **离线**：权重内置（雷达 v6.2 852KB + 裁判 v5 285KB），无网络请求、无隐私外泄
...
|---|---|---|---|
| 关键词规则 | 1.8% | 0% | 55.0% |
| **雷达 v6.2（端内 0.04ms）** | **92.7%** | **3.1%** | **95.0%** |
...
**真实香港案例卷（零样本）**：12 个真实 HK 骗案家族（ADCC/警方/新闻逐案重构并标注出处）+ 8 条真实机构通知——雷达 v6.2 **召回 100%（全部高危档）**、误报 12.5%；关键词规则召回 **0%**；大模型 few-shot 同为 100% 但需秒级联网。
